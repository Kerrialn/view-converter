...
final class LoopsPattern implements BladePatternInterface
{
    public function apply(string $content): string
    {
        ...
        $content = preg_replace_callback('/@foreach\s*\((.+?)\s+as\s+(.+?)\)/', function ($m) {
            return BladeExpressionHelper::forStatement(trim($m[1]), trim($m[2]));
        }, $content);
        $content = str_replace('@endforeach', '{% endfor %}', $content);

        // Remaining @forelse without @empty (degenerate case)
        $content = preg_replace_callback('/@forelse\s*\((.+?)\s+as\s+(.+?)\)/', function ($m) {
            return BladeExpressionHelper::forStatement(trim($m[1]), trim($m[2]));
        }, $content);
        $content = str_replace('@endforelse', '{% endfor %}', $content);

        // @for — no clean Twig equivalent
        $content = preg_replace_callback('/@for\s*\((.+?)\)/', function ($m) {
            return '{# TODO: @for(' . trim($m[1]) . ') - review manually #}';
        }, $content);
        $content = str_replace('@endfor', '{% endfor %}', $content);

        // @while — no Twig equivalent
        $content = preg_replace_callback('/@while\s*\((.+?)\)/', function ($m) {
            return '{# TODO: while (' . BladeExpressionHelper::convertExpr(trim($m[1])) . ') - no Twig equivalent #}';
        }, $content);
